#!/usr/bin/env python3
"""Example: RT2D Cartesian oblique tracing on an IRI profile."""
...
def _run(cfg, event_time: dt.datetime, mode: str, formulation: str) -> None:
    workers = int(getattr(cfg, "worker", 1))
    # Force a ground-referenced profile for this demo (0 km to end_height_km).
    ...
    logger.info(
        "RT2D profile altitude bounds: {:.1f} to {:.1f} km (n={})",
    ...
    out_file = Path.cwd() / "docs/examples/figures/rt2d_iri_cartesian_ray_paths.png"
    _plot_density_and_rays(profile, rays, out_file)

    logger.info(
        "Completed RT2D Cartesian run: rays={}, mode={}, model={}",
        len(rays),
        mode,
        formulation,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run RT2D Cartesian oblique rays using IRI profile and plot rays."
    )
    parser.add_argument(
        "--config",
        default=None,
