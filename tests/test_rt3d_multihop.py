...
@pytest.mark.skip(
    reason="3D spherical gradient ODE is too slow for CI — skipped pending solver optimisation"
)
...
def test_specular_elevation_preserved_cartesian():
    ...
    if out.nhops_completed >= 2:
        ...
        vx_f = float(out.vx[-1])
        vz_f = float(out.vz[-1])
        elev_terminal = float(np.degrees(np.arctan2(abs(vz_f), np.sqrt(vx_f**2))))
