...
#
#  Release targets:
#    make check   VERSION=X.Y.Z      Consistency check only (no changes)
#    make release VERSION=X.Y.Z      Full interactive release pipeline
#
...
help:
	@echo ""
	@echo "  hfpytrace — available targets"
	@echo "  ═══════════════════════════════════════════════════════════════"
	@echo "  Everyday"
	@echo "  ───────────────────────────────────────────────────────────────"
...
clean:
	...
	rm -rf hfpytrace.egg-info/ *.egg-info/
	rm -rf .eggs/
	rm -rf junit.xml .coverage coverage.xml coverage.json htmlcov/ .pytest_cache/
...
install-dev:
	@set -euo pipefail
	echo ""
	echo "  Installing hfpytrace in editable mode with dev dependencies..."
	pip install -e .[dev]
	echo "  ✔  Done."
	echo ""
...
build-dist:
	...
	isort -rc -m 3 .
	black .
	rm -rf dist/ build/ hfpytrace.egg-info/ *.egg-info/
...
#   Usage:  make sync
#           make sync MSG="feat: add new feature"
# --------------------------------------------------------------------------- #
sync:
	@set -euo pipefail
...
check:
	...
	if [[ "$(VERSION)" == "__UNSET__" ]]; then
	  echo ""
	  echo "  ERROR: No version provided.  Usage:  make check VERSION=0.0.4"
	  echo ""
	  exit 1
	fi
	if ! echo "$(VERSION)" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$$'; then
	  echo "ERROR: VERSION must be MAJOR.MINOR.PATCH (e.g. 0.0.4), got '$(VERSION)'"
	  exit 1
	fi
	echo ""
	...
	INIT_VER=$$(grep  -oP '(?<=__version__ = ")[^"]+' hfpytrace/__init__.py 2>/dev/null || echo "__missing__")
...
release:
	...
	if [[ "$(VERSION)" == "__UNSET__" ]]; then
	  ...
	  echo "  Usage:  make release VERSION=X.Y.Z   (e.g.  make release VERSION=0.0.4)"
	  echo ""
	  exit 1
	fi
	if ! echo "$(VERSION)" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$$'; then
	...
	echo ""
	echo "╔══════════════════════════════════════════════════════════════╗"
	echo "║           hfpytrace  —  Release $(FULL_VER)                "
	echo "║  Git tag  : $(TAG)                                          "
	echo "║  Remote   : git@github.com:shibaji7/trace.git               "
	echo "╚══════════════════════════════════════════════════════════════╝"
	echo ""
	  ...
	  if confirm "Apply version updates to setup.py and __init__.py?"; then
	    ...
	    if [[ "$$NEED_INIT" == "true" ]]; then
	      if [[ "$${INIT_VER}" == "__missing__" ]]; then
	        echo "__version__ = \"$(FULL_VER)\"" >> hfpytrace/__init__.py
	        echo "  ✔  __version__ added to hfpytrace/__init__.py."
	      else
	        sed -i "s/__version__ = \"[^\"]*\"/__version__ = \"$(FULL_VER)\"/" hfpytrace/__init__.py
	        echo "  ✔  hfpytrace/__init__.py updated."
	      fi
	    fi
	  else
	    skip_step
	...
	if ! command -v gh &>/dev/null; then
	  ...
	  RELEASE_NOTES="## hfpytrace $(FULL_VER)"$$'\n\n'"HF propagation ray tracing tools."$$'\n\n'"### Installation"$$'\n\n'"```bash"$$'\n'"pip install hfpytrace==$(FULL_VER)"$$'\n'"```"$$'\n\n'"### Changelog"$$'\n\n'"- Version bump to $(FULL_VER)"
	  echo "  Will run:"
	  echo "    gh release create $(TAG) --title \"hfpytrace $(FULL_VER)\" --notes \"...\""
	  echo ""
	  if confirm "Create GitHub release $(TAG)?"; then
	    gh release create $(TAG) \
	        --title "hfpytrace $(FULL_VER)" \
	        --notes "$$RELEASE_NOTES"
	...
	if [[ "$${SKIP_BUILD:-false}" != "true" ]]; then
	  ...
	  echo ""
	  echo "  Produces dist/hfpytrace-$(FULL_VER).tar.gz"
	  echo "           dist/hfpytrace-$(FULL_VER)-py3-none-any.whl"
	  echo ""
	...
	if [[ "$${SKIP_TWINE:-false}" != "true" ]]; then
	  ...
	  if ! python -m twine check dist/*; then
	    echo ""
	    echo "  ⚠  twine check failed — usually an outdated twine or a metadata"
	    echo "     issue (e.g. License-File field from setuptools ≥ 68)."
	...
	if [[ "$${SKIP_TWINE:-false}" != "true" ]]; then
	  ...
	  if confirm "Upload dist/* to PyPI?"; then
	    ...
	    echo "     https://pypi.org/project/hfpytrace/$(FULL_VER)/"
	  else
	    skip_step
	  fi
	fi
	...
	echo "║  Tag    : $(TAG)                                            "
	echo "║  PyPI   : https://pypi.org/project/hfpytrace/$(FULL_VER)/  "
	echo "║  GitHub : https://github.com/shibaji7/trace/releases        "
	echo "╚══════════════════════════════════════════════════════════════╝"
	echo ""
